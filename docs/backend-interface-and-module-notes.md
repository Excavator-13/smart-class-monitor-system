...
- **返回格式**：XML
- **解析逻辑**：查找同名 `<stream>`，其 `<clients>` 中至少一个 `<client>` 包含 `<publishing/>` 表示正在推流
...
| 检测方 | 检测方式 | 用途 | 频率 |
| --- | --- | --- | --- |
| SpringBoot `/streams/{stream_id}/status` | 请求 Nginx `/stat` XML 解析发布者 `<publishing/>` | 前端展示视频源在线/离线状态 | 低频轮询（按需或 30s+） |
...
| AI 调用接口（读） | `GET /streams` `GET /zones` `GET /rules` `GET /score-config` `GET /students/face-features` | AI 启动/刷新时拉取配置和人脸特征（复用前端接口或专用接口） |
...
| 视频源推流状态 | `online`、`offline`、`unknown` | 🟢 |
...
| 状态 | 接口 | 方法 | 使用场景 | 主要入参 | 主要返回 | 备注 |
| --- | --- | --- | --- | --- | --- | --- |
| 🟢 | `/streams` | GET | 获取可切换视频源列表 | `status`、`keyword` | 视频源列表；RTMP 地址按权限返回 | 管理员和内部服务可见 `rtmp_url`，教师响应不暴露该字段 |
...
| 🟢 | `/streams/{stream_id}/status` | GET | 查询当前视频源推流状态 | `stream_id` | `online`、`status`、`uptime` | 解析标准 `<publishing/>`；状态页不可用时返回 `unknown` |
...
| 🟡 | `/streams/{stream_id}/preview-url` | GET | 获取前端播放地址 | `stream_id` | `mjpeg_url`、`rtmp_url`、`hls_url` | `rtmp_url` 仅管理员和内部服务返回；SpringBoot 不代理视频流 |
...
>
> 行为规则不包含告警等级。一个规则可能产生多个事件（如危险区接近、进入、停留），各事件的等级由 `/score-config` 独立配置。
...
| 🟢 | `/score-config` | GET | 查询具体事件等级与评分 | 无 | `alert_type`、`label`、`level`、`score`、`note` | `alert_type` 使用 AI 实际 `event_type` |
| 🟢 | `/score-config/{id}` | PUT | 修改事件等级与评分 | query: `level`、`score`、`label`、`note` | 更新后的配置 | 仅管理员；修改后通知 AI 重载 `event_configs` |
...
| `level` | string | 否 | AI 兼容字段；SpringBoot 最终按 `alert_type` 的事件配置校正为 `info` / `warning` / `high` |
...
### 6.4.1 获取异常事件等级与评分配置（AI → SpringBoot）

```http
GET /score-config
```

AI 按实际 `event_type` 缓存事件配置，并在统一事件确认入口应用 `level`。规则配置不再携带或决定告警等级。

主要返回字段：`alert_type`、`label`、`level`、`score`、`note`。
...
| 🟢 | `/score-config` | GET | 获取具体事件等级与评分 | 是 | AI 使用 `alert_type -> level` 映射 |
...
{
  "stream_id": "classroom_01",
  "reload_items": ["streams", "zones", "rules", "event_configs"]
}
```
...
| `score_config` | 具体异常事件等级与评分 | `id`、`alert_type`、`label`、`level`、`score`、`note` | `/score-config` | 🟢 |
...
- `score_config.alert_type`：与 AI `event_type` 一一对应；`face_recognized` 属于正常事件，不进入告警配置
